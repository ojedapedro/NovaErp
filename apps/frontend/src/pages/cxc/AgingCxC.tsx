...
export const AgingCxC: React.FC = () => {
  const { data, isLoading } = useQuery({
    queryKey: ["cxc-aging"],
    queryFn: cxcApi.getAging,
  });
  ...
  const columns = [
    ...
    { title: "Días Vencido", dataIndex: "diffDays", key: "diffDays", align: "right" as const },
    { 
      title: "Rango", 
      dataIndex: "category", 
      key: "category",
  ...
  return (
    <div>
      <Title level={2}>Antigüedad de Cuentas por Cobrar (Aging)</Title>

      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={4}>
          <Card bordered={false}>
            <Statistic title="0 a 30 días (Sano)" value={summary["0-30"]} precision={2} prefix="$" valueStyle={{ color: '#3f8600' }} />
          </Card>
        </Col>
        <Col span={4}>
          <Card bordered={false}>
        ...
        <Col span={4}>
          <Card bordered={false}>
            <Statistic title="Más de 90 días (Peligro)" value={summary["+90"]} precision={2} prefix="$" valueStyle={{ color: '#cf1322' }} />
          </Card>
        </Col>
        <Col span={8}>
          <Card bordered={false} style={{ background: '#fafafa' }}>
            <Statistic title="DEUDA TOTAL POR COBRAR" value={summary.total} precision={2} prefix="$" valueStyle={{ fontWeight: 'bold' }} />
          </Card>
        </Col>
      </Row>
